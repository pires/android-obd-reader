        ...
        Preference.OnPreferenceChangeListener
{
    ...
    {
        String periodString = prefs.getString(ConfigFragment.UPDATE_PERIOD_KEY, "4"); // 4 as in seconds
    ...
    {
        String veString = prefs.getString(ConfigFragment.VOLUMETRIC_EFFICIENCY_KEY, ".85");
        double ve = 0.85;
        try
        {
    ...
    {
        String edString = prefs.getString(ConfigFragment.ENGINE_DISPLACEMENT_KEY, "1.6");
        double ed = 1.6;
        try
        {
    ...
    {
        String maxStr = prefs.getString(ConfigFragment.MAX_FUEL_ECON_KEY, "70");
        double max = 70;
        try
        {
    ...
    public void onCreate(Bundle savedInstanceState)
    {
        ...
        String[] prefKeys = new String[]{
                ENGINE_DISPLACEMENT_KEY, VOLUMETRIC_EFFICIENCY_KEY, UPDATE_PERIOD_KEY, MAX_FUEL_ECON_KEY
        };
        ...
        {
            CheckBoxPreference cpref = new CheckBoxPreference(getActivity());
        ...
        if (mBtAdapter == null)
        {
            ...
            Toast.makeText(getActivity(), "This device does not support Bluetooth.", Toast.LENGTH_LONG);
        ...
        listBtDevices.setOnPreferenceClickListener(new Preference.OnPreferenceClickListener()
        {
            public boolean onPreferenceClick(Preference preference)
            {
                ...
                {
                    Toast.makeText(getActivity(), "This device does not support Bluetooth or it is disabled.", Toast.LENGTH_LONG);
                    return false;
                }
                return true;
            }
    ...
    {
        if (UPDATE_PERIOD_KEY.equals(preference.getKey()) || VOLUMETRIC_EFFICIENCY_KEY.equals(preference.getKey()) || ENGINE_DISPLACEMENT_KEY.equals(preference.getKey()) || UPDATE_PERIOD_KEY.equals(preference.getKey()) || MAX_FUEL_ECON_KEY.equals(preference.getKey()))
        {
            try
            {
            ...
            {
                Toast.makeText(getActivity(), "Couldn't parse '" + newValue.toString() + "' as a number.", Toast.LENGTH_LONG).show();
            }
        }
        return false;
    }
